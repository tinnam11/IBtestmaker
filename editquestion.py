...
class Editquestion:
    def __init__(self,window, question_id):
        ...
        # Questions label
        self.questionlabel = ttk.Label(self.my_frame, text='Edit question', font=('', 30))
        ...
        # --------------------------------------
        self.savebt = ttk.Button(self.my_frame, text='save', style='success.TButton', command=lambda:self.savequestion(question_id))
...
# copy text function
    ...
    def savequestion(self, a):
        ...
        if len(self.answer)>0:
            ...
            self.answer=""
        print(self.tempstr[0])

        #update query to make changes to the question
        self.sql="UPDATE newpastpaper SET question = '" +self.question+"', answer_choice_paper1 " \
         "= '" +self.answerchoice+"',mark_scheme='" +self.markscheme+"' WHERE No = '" + a + "'"
        #excecuting the update query
        self.mycursor.execute(self.sql)
        self.mydb.commit()
        self.message = messagebox.showinfo('', 'successfully edited the question!')
        self.root.destroy()
