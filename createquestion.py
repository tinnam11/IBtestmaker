...
class Newquestion:
    def __init__(self,window):
        ...
        # Questions label
        self.questionlabel = ttk.Label(self.my_frame, text='New question', font=('', 30))
        ...
        self.tagframe = ttk.Labelframe(self.toolbar_frame, text='Tags')
        self.tagframe.grid(row=0, column=4, padx=10)
        ...
        self.levelframe = ttk.Labelframe(self.toolbar_frame, text='Level', padding=(20, 10, 10, 5))
        ...
        self.paperframe = ttk.Labelframe(self.toolbar_frame, text='Paper style', padding=(20, 10, 10, 5))
...
#subscipt text function
    ...
    #function to insert special characters into the entry field
    def specialcharacter(self,character):
        self.my_text.insert(END,character)
    #popup page for the special characters
        ...
        for i in range(self.row):
            for y in range(self.column):
                ...
                if self.count < len(self.list):
                    ...
                    ttk.Button(self.specialcharframe, text=self.list[self.count], command=lambda k=self.count: self.specialcharacter(self.list[k])).grid(
    ...
    def savequestion(self):
        ...
        self.tag=self.tag1check('event')
        self.level=self.levelcheck()
        self.paper=self.paperstylecheck()
        ...
        if len(self.dupresults) > 0 and len(self.dupresults2) > 0 and len(self.dupresults3) > 0:
            ...
            #inserting question into the database query
            self.sql = "INSERT INTO newpastpaper (question, answer_choice_paper1, image_path, mark_scheme,tag,last_used, Answer_space, Parent,level, Exam_paper_style) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            print(self.sql)
            #the insert variables for each column of the database
            self.insertvar = (self.question, self.answerchoice,self.image, self.markscheme, self.tag,self.date,self.answer,"0", self.level,self.paper)
            #SQL executing query
